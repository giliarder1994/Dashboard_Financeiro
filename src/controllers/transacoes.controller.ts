...
export async function listar(
  ...
  try {
    ...
    const [rows] = await pool.promise().query<import("mysql2").RowDataPacket[]>(sql, params);
...
export async function criar(
  ...
  try {
    const usuario_id = (req as ReqAuth).usuario.id;
    const { descricao, valor, tipo, data, categoria_id } = req.body as {
      descricao?: string;
      valor?: number;
      tipo?: string;
      data?: string;
    ...
    const [resultado] = await pool.promise().query<import("mysql2").ResultSetHeader>(
      ...
      [descricao, valor, tipo as TipoTransacao, data, usuario_id, categoria_id]
    );
